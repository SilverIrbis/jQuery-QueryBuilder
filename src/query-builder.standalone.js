...
<div id="{{= it.rule_id }}" class="rule-container"> \
  ...
  {{? it.settings.display_errors }} \
    <div class="error-container"><i class="{{= it.icons.error }}"></i></div> \
  {{?}} \
  <div class="rule-visibility-container"><input type="checkbox"></div> \
...
{{ var optgroup = null; }} \
<select name="{{= it.rule.id }}_filter"> \
...
{{?}} \
{{ var optgroup = null; }} \
<select class="{{? it.operators.length === 1 }}hide{{?}}" name="{{= it.rule.id }}_operator"> \
